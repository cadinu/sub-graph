...
import { updatePairDayData, updateTokenDayData, updateCadinuDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  cadinu.totalLiquidityBNB = cadinu.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  cadinu.totalLiquidityBNB = cadinu.totalLiquidityBNB.plus(trackedLiquidityBNB);
  cadinu.totalLiquidityUSD = cadinu.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  cadinu.totalTransactions = cadinu.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let cadinu = CadinuFactory.load(FACTORY_ADDRESS);
  cadinu.totalVolumeUSD = cadinu.totalVolumeUSD.plus(trackedAmountUSD);
  cadinu.totalVolumeBNB = cadinu.totalVolumeBNB.plus(trackedAmountBNB);
  cadinu.untrackedVolumeUSD = cadinu.untrackedVolumeUSD.plus(derivedAmountUSD);
  cadinu.totalTransactions = cadinu.totalTransactions.plus(ONE_BI);
  ...
  cadinuDayData.dailyVolumeUSD = cadinuDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  cadinuDayData.dailyVolumeBNB = cadinuDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  cadinuDayData.dailyVolumeUntracked = cadinuDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  cadinuDayData.save();
