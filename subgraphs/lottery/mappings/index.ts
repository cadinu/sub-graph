...
export function handleLotteryOpen(event: LotteryOpen): void {
  ...
  lottery.ticketPrice = toBigDecimal(event.params.priceTicketInCadinu);
...
export function handleTicketsPurchase(event: TicketsPurchase): void {
  ...
  if (user === null) {
    ...
    user.totalCadinu = ZERO_BD;
    user.block = event.block.number;
    user.timestamp = event.block.timestamp;
    user.save();
  }
  user.totalTickets = user.totalTickets.plus(event.params.numberTickets);
  user.totalCadinu = user.totalCadinu.plus(event.params.numberTickets.toBigDecimal().times(lottery.ticketPrice));
  user.save();
...
export function handleTicketsClaim(event: TicketsClaim): void {
  ...
  if (user !== null) {
    user.totalCadinu = user.totalCadinu.plus(toBigDecimal(event.params.amount));
    user.save();
  }
